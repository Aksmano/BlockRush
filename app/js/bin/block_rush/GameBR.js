  ...
  constructor(brickType = "normic_brick") {
    ...
    Specs.BrickC.loadBrick("../../models/normic_brick.gltf", modeldata => {
      ...
      modeldata.position.set(
        Specs.blockRushBrickDist[Specs.brickType] * Specs.scale * 1000,
        Specs.blockRushBrickDist[Specs.brickType] * Specs.scale * 1000,
        Specs.blockRushBrickDist[Specs.brickType] * Specs.scale * 1000
      );
    ...
    const render = () => {
      ...
      if (Specs.isForWin) {
        if (this.timestamp == 60) {
          if (
            parseInt(document.getElementById("timeNumber").innerText) - 1 <=
              0 &&
            Specs.pointsToWin > Specs.playerPoints
          ) {
            document.body.removeChild(document.getElementById("root"));
            document.getElementById("backg").innerText =
              "You lost :( Press F5 to play again";
            cancelAnimationFrame(render);
          } else if (Specs.pointsToWin <= Specs.playerPoints) {
            document.body.removeChild(document.getElementById("root"));
            document.getElementById("backg").innerText =
              "You won! Press F5 to play again";
            cancelAnimationFrame(render);
          } else {
            document.getElementById("timeNumber").innerText =
              parseInt(document.getElementById("timeNumber").innerText) - 1;
            this.timestamp = 0;
          }
        } else this.timestamp++;
